...
class Querier extends BaseQuerier {
  defineSchema(schema) {
    // filtering
    schema.filter('q', '=')
    schema.filter('IDCard', operators, { field: 'BL_IDCards.IDCard' })

    // sorting
    schema.sort('IDCard', { field: 'BL_IDCards.IDCard' })
  ...
  'filter:q[=]'(builder, { value }) {
    ...
    return builder.where(function () {
      this.orWhere('BL_IDCards.IDCard', 'like', value)
    })
  }
  ...
  async run() {
    ...
    if (_.get(this.query, 'page.number')) {
      builder.orderBy('BL_IDCards.IDCard', 'asc')
    }
